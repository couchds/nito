...
Use the Create page to make a prompt-backed sample with a two-step flow: enter the character prompt, then choose the skeleton/body type from the visual cards. The original prompt and generated front/left/right/back OpenAI prompts are stored in the sample workflow state. Each sample detail page shows the expected rig family, bone chains, pipeline state, reference images, Blender review renders, and downloaded GLB/GLTF model when those artifacts exist. Batches remain candidate training sets, and a sample can appear in multiple batches because membership is computed from saved batch summaries.
